...
public abstract class BasePhysicalTable implements PhysicalTable {
    ...
    public Map<Column, Set<Interval>> getAllAvailableIntervals() {
        return getAvailability().getAllAvailableIntervals().entrySet().stream()
                .filter(entry -> getSchema().containsColumn(entry.getKey()))
                .collect(Collectors.toMap(entry -> getSchema().getColumn(entry.getKey()).get(), Map.Entry::getValue));
    }
